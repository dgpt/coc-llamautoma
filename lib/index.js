...
var require_event_target_shim = __commonJS((exports2, module2) => {
  Object.defineProperty(exports2, "__esModule", { value: true });
...
var require_abort_controller = __commonJS((exports2, module2) => {
  Object.defineProperty(exports2, "__esModule", { value: true });
...
`);
    const contents = await Promise.all(lines.filter((line) => line.trim()).map((line) => this.parseStreamingLine(line)));
...
`).trim();
  ...
  async makeRequest(endpoint, body) {
    ...
    try {
      const response = await import_node_fetch.default(`${this.serverUrl}${endpoint}`, {
        method: "POST",
        headers: {
  ...
  async chat(message) {
    const config = import_coc.workspace.getConfiguration("llamautoma");
    ...
    const body = this.createRequestBody(messages, {
      threadId,
      modelName: config.get("model", "qwen2.5-coder:7b"),
      maxFileSize: config.get("maxFileSize", 8192)
    });
    const response = await this.makeRequest("/v1/chat", body);
    const result = await this.processResponse(response);
    if (result.content && !result.content.includes("No response content received") && !result.content.includes("Error processing response")) {
  ...
  async edit(message) {
    ...
    const response = await this.makeRequest("/v1/edit", body);
    return this.processResponse(response);
  }
  async compose(message) {
    const config = import_coc.workspace.getConfiguration("llamautoma");
    const messages = [
    ...
    const response = await this.makeRequest("/v1/compose", body);
    return this.processResponse(response);
  }
  async sync() {
    const config = import_coc.workspace.getConfiguration("llamautoma");
    ...
    const body = this.createRequestBody(messages, {
      modelName: config.get("model", "qwen2.5-coder:7b"),
      maxFileSize: config.get("maxFileSize", 8192),
      root: import_coc.workspace.root,
...
class LlamautomaCommands {
  ...
  getOrCreateChannel(name) {
    if (!this.channels.has(name)) {
      const channel = import_coc2.window.createOutputChannel(name);
  ...
  async appendToChannel(channel, text) {
    if (!text || !text.trim())
      return;
    if (text.includes('"type":') || text.includes('"content":') || text.startsWith("{") && !text.startsWith(">> "))
      return;
    const nvim = import_coc2.workspace.nvim;
  ...
  async handleError(error, command) {
    ...
    import_coc2.window.showErrorMessage(`${command} failed: ${message}`);
    const channel = await this.showChannel("Llamautoma Error");
    await this.appendToChannel(channel, `Error details: ${message}`);
    if (error instanceof Error && error.stack) {
      await this.appendToChannel(channel, `
Stack trace:
${error.stack}`);
    }
  }
  async chat() {
    try {
      ...
      const bufnr = await import_coc2.workspace.nvim.call("bufadd", ["llamautoma://chat"]);
      await import_coc2.workspace.nvim.call("bufload", [bufnr]);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&buftype", "nofile"]);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&swapfile", 0]);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&modifiable", 1]);
      await import_coc2.workspace.nvim.command(`buffer ${bufnr}`);
      const input = await import_coc2.window.requestInput("Enter your message: ");
      if (!input)
        return;
      await import_coc2.workspace.nvim.call("append", [0, ["", "Sending request...", ""]]);
      const response = await this.client.chat(input);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&modifiable", 1]);
      await import_coc2.workspace.nvim.call("deletebufline", [bufnr, 1, "$"]);
      const content = response.content || "No response received";
      await import_coc2.workspace.nvim.call("append", [0, ["", "Response:", content, ""]]);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&modifiable", 0]);
      await import_coc2.workspace.nvim.call("setbufvar", [bufnr, "&readonly", 1]);
    } catch (error) {
      if (error instanceof Error) {
        import_coc2.window.showErrorMessage(`Error in chat command: ${error.message}`);
      } else {
        import_coc2.window.showErrorMessage("An unknown error occurred in chat command");
      }
    }
  }
  async edit() {
    try {
      const doc = await import_coc2.workspace.document;
      if (!doc) {
        import_coc2.window.showErrorMessage("No active document");
        return;
      }
      const input = await import_coc2.window.requestInput("Enter your edit request:");
      if (!input)
        return;
      const channel = await this.showChannel("Llamautoma Edit");
      await this.appendToChannel(channel, "Sending edit request...");
      const response = await this.client.edit(input);
      if (response.edits) {
        for (const edit of response.edits) {
          if (edit.file === doc.uri) {
            const buffer = await import_coc2.workspace.nvim.buffer;
            const lines = edit.content.split(`
`);
            await buffer.setLines(lines, { start: 0, end: doc.lineCount });
            await this.appendToChannel(channel, "Edit applied successfully");
          }
        }
      } else {
        await this.appendToChannel(channel, "No edits received from server");
      }
    } catch (error) {
      await this.handleError(error, "Edit");
    }
  }
  async compose() {
    try {
      const filename = await import_coc2.window.requestInput("Enter the filename:");
      if (!filename)
        return;
      const input = await import_coc2.window.requestInput("Enter your compose request:");
      if (!input)
        return;
      const channel = await this.showChannel("Llamautoma Compose");
      await this.appendToChannel(channel, "Sending compose request...");
      const response = await this.client.compose(input);
      if (response.files) {
        for (const file of response.files) {
          await import_coc2.workspace.nvim.command(`new ${file.path}`);
          const buffer = await import_coc2.workspace.nvim.buffer;
          const lines = file.content.split(`
`);
          await buffer.setLines(lines, { start: 0, end: -1 });
          await this.appendToChannel(channel, `Created file: ${file.path}`);
        }
      } else {
        await this.appendToChannel(channel, "No files received from server");
      }
    } catch (error) {
      await this.handleError(error, "Compose");
    }
  }
  async sync() {
    try {
      ...
      if (response.status === "success") {
        import_coc2.window.showInformationMessage("Workspace synchronized successfully");
...
async function activate(context) {
  import_coc3.window.showInformationMessage("Llamautoma activated!");
  const config = import_coc3.workspace.getConfiguration("llamautoma");
  console.log("Raw config:", config);
  const serverUrl = config.get("url") ?? "http://localhost:3000";
  const timeout = config.get("timeout", 30000);
  const secret = config.get("secret", "");
  const model = config.get("model", "qwen2.5-coder:7b");
  const maxFileSize = config.get("maxFileSize", 8192);
  ...
  context.subscriptions.push(import_coc3.commands.registerCommand("llamautoma.chat", async () => {
    console.log("Executing chat command");
    await llamautomaCommands.chat();
  }), import_coc3.commands.registerCommand("llamautoma.edit", async () => {
    console.log("Executing edit command");
    await llamautomaCommands.edit();
  }), import_coc3.commands.registerCommand("llamautoma.compose", async () => {
    console.log("Executing compose command");
    await llamautomaCommands.compose();
  }), import_coc3.commands.registerCommand("llamautoma.sync", async () => {
